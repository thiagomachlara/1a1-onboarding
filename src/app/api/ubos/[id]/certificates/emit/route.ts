...
export async function POST(
  ...
  try {
    ...
    if (ubo.dob) {
      ...
      if (dobStr.includes('-')) {
        // Formato: YYYY-MM-DD (manter formato ISO)
        birthdate = dobStr.split('T')[0]; // Remove hora se tiver
      } else {
        // Fallback: usar Date e formatar para ISO
      ...
      console.log('[UBO_EMIT] ubo.dob é null/undefined!');
    }
    ...
    try {
      console.log('[UBO_EMIT] Chamando InfoSimples:', { certificate_type, cpf, birthdate, nome });
